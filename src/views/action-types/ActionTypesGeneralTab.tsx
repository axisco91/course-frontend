...
// ✅ APIs
import { createActionType, editActionType, getActionType } from 'src/api/api'
...
const ActionTypesGeneralTab: React.FC<ActionTypeGeneralTabProps> = ({
  open,
  mode,
  actionTypeId,
  onLoaded,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await getActionType(actionTypeId)

        if (cancelled) return

        const l = res.data?.data?.action_type ?? null
        if (!l) throw new Error('Action Type payload not found')
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      if (mode === 'create') {
        const response = await createActionType(formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())

          const newId = response.data?.data?.action_type?.id ?? response.data?.data?.id
          if (newId) {
            if (actionTypeActions?.setId) dispatch(actionTypeActions.setId(Number(newId)))
            if (actionTypeActions?.openModal) {
              dispatch(actionTypeActions.openModal({ mode: 'edit', actionTypeId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && actionTypeId) {
        const response = await editActionType(actionTypeId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default ActionTypesGeneralTab
