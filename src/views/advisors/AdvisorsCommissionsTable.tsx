...
import { getAdvisorCommissions } from 'src/api/api'
...
const AdvisorsCommissionsTable = ({ open, advisorId }: Props) => {
  ...
  const canViewCommissions = Array.isArray(userPermissions) && userPermissions.includes('read.advisors')
    ...
    if (canViewCommissions) {
      base.push({
        ...
        renderCell: (params: GridRenderCellParams) => {
          ...
          return (
            <Tooltip title={t('View commission')} placement='top'>
              <IconButton
                size='small'
                onClick={e => {
                  e.stopPropagation()
                  ...
                  dispatch(advisorCommissionActions.setId(Number(cid)))
                  dispatch(advisorCommissionActions.setCurrentAdvisorCommission(params.row))
                  dispatch(advisorCommissionActions.openModal({ mode: 'view', advisorCommissionId: Number(cid) }))
                }}
              >
  ...
  const fetchCommissions = useCallback(async () => {
    ...
    try {
      ...
      const res = await getAdvisorCommissions(advisorId, {
      ...
      setRows(res.data?.data?.advisor_commissions ?? [])
    } catch (error) {
...
export default AdvisorsCommissionsTable
