...
import { getAdvisorCompanies } from 'src/api/api'
...
const AdvisorsCompaniesTable = ({ open, advisorId, active = true }: Props) => {
  ...
  const canViewCompany = Array.isArray(userPermissions) && userPermissions.includes('read.companies')
            ...
            <Tooltip title={t('View company')} placement='top'>
              <IconButton
                ...
                onClick={e => {
                  ...
                  dispatch(companyActions.setId(Number(cid)))

                  // ✅ tú ajustas el reducer/nombre si hace falta
                  dispatch(companyActions.openModal({ mode: 'view', companyId: Number(cid) }))
                }}
              >
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      const res = await getAdvisorCompanies(advisorId, {
      ...
      setRows(res.data?.data?.companies ?? [])
    } catch (error) {
      if (myReqId === reqIdRef.current) handleErrorRef.current(error, logoutRef.current)
    } finally {
      if (myReqId === reqIdRef.current) setLoading(false)
    }
  }, [open, active, advisorId, paginationModel.page, paginationModel.pageSize, sortModel])
...
export default AdvisorsCompaniesTable
