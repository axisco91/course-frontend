...
import BillsGeneralTab from './BillsGeneralTab'
import BillsStudentsTable from './BillsStudentsTable'

// ✅ Usa tus componentes reales de Bills
...
const BillsModal: React.FC<BillsModalProps> = ({ open, onClose, mode, billId = null }) => {
  ...
  const hasId = Boolean(billId)
  const studentsEnabled = !isCreate && hasId

  // reset tab + name when opening or changing bill
  useEffect(() => {
    if (!open) return
    setTab(0)
    setBillName('')
  ...
  const handleLoaded = useCallback((b: any) => {
    // ✅ Bill title (ajusta el campo a tu modelo real)
    setBillName(`${b?.name ?? b?.title ?? ''}`.trim())
  }, [])
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Bill') : billName || t('Bill')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={1}>
          {open && studentsEnabled && tab === 1 && <BillsStudentsTable open={open} billId={billId} />}
        </TabPanel>
      </DialogContent>
    </Dialog>
  )
