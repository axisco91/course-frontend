...
const CentersFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).center.filters)

  const handleCreate = () => {
    dispatch(centerActions.setId(null))

    // ✅ abre modal en create
    dispatch(centerActions.openModal({ mode: 'create', centerId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(centerActions.setFilter({ key, value }))
