...
import CentersGeneralTab from './CentersGeneralTab'
...
const CentersModel: React.FC<CenterModalProps> = ({ open, onClose, mode, centerId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Center') : t('Center')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <CentersGeneralTab open={open} mode={effectiveMode} centerId={centerId ?? null} onClose={onClose} />
        </TabPanel>
      </DialogContent>
    </Dialog>
  )
