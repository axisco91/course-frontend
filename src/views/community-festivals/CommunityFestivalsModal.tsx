...
import CommunityFestivalsGeneralTab from './CommunityFestivalsGeneralTab'
...
interface CommunityFestivalsModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const CommunityFestivalsModal: React.FC<CommunityFestivalsModalProps> = ({
  open,
  onClose,
  mode,
  communityFestivalId = null
}) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Community Festival') : t('Community Festival')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <CommunityFestivalsGeneralTab
            open={open}
            mode={effectiveMode}
            communityFestivalId={communityFestivalId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default CommunityFestivalsModal
