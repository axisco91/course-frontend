...
const CommunitiesFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).community.filters)

  const handleCreate = () => {
    dispatch(communityActions.setId(null))

    // ✅ abre modal en create
    dispatch(communityActions.openModal({ mode: 'create', communityId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(communityActions.setFilter({ key, value }))
