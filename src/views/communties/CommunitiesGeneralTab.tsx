...
// ✅ APIs
import { createCommunity, editCommunity, getCommunity } from 'src/api/api'
...
interface CommunitiesGeneralTabProps {
  open: boolean
  mode: Mode
...
const CommunitiesGeneralTab: React.FC<CommunitiesGeneralTabProps> = ({
  open,
  mode,
  communityId,
  onLoaded,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const l = res.data?.data?.community ?? null
        if (!l) throw new Error('Community payload not found')
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (response.data?.success) {
          ...
          const newId = response.data?.data?.community?.id ?? response.data?.data?.id
          if (newId) {
            if (communityActions?.setId) dispatch(communityActions.setId(Number(newId)))
            if (communityActions?.openModal) {
              dispatch(communityActions.openModal({ mode: 'edit', communityId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && communityId) {
        const response = await editCommunity(communityId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default CommunitiesGeneralTab
