...
// ✅ APIs
import { createCompanyType, editCompanyType, getCompanyType } from 'src/api/api'
...
interface CompanyTypeGeneralTabProps {
  open: boolean
  mode: Mode
...
const CompanyTypesGeneralTab: React.FC<CompanyTypeGeneralTabProps> = ({
  open,
  mode,
  companyTypeId,
  onLoaded,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await getCompanyType(companyTypeId)
        if (cancelled) return

        const l = res.data?.data?.company_type ?? null
        if (!l) throw new Error('Company Type payload not found')
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (response.data?.success) {
          ...
          const newId = response.data?.data?.company_type?.id ?? response.data?.data?.id
          if (newId) {
            if (companyTypeActions?.setId) dispatch(companyTypeActions.setId(Number(newId)))
            if (companyTypeActions?.openModal) {
              dispatch(companyTypeActions.openModal({ mode: 'edit', companyTypeId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && companyTypeId) {
        const response = await editCompanyType(companyTypeId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default CompanyTypesGeneralTab
