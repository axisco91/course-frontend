...
// ✅ endpoint real
import { getCourseStudents } from 'src/api/api'
...
const CoursesStudentsTable = ({ open, courseId, active = true }: Props) => {
  ...
  const columns = useMemo(() => {
    ...
    if (canViewStudents) {
      base.push({
        ...
        renderCell: (params: GridRenderCellParams) => {
          // IMPORTANT: adapta esto a tu payload real:
          // a veces viene student_id en registrations
                ...
                <Tooltip title={t('Edit')} placement='top'>
                  <IconButton
                    ...
                    onClick={e => {
                      e.stopPropagation()
                      blurActiveElement()
                      dispatch(registrationActions.setId(Number(params.row?.registration_id)))
                      dispatch(registrationActions.openModal({ mode: 'edit' }))
                    }}
                  >
                ...
                <Tooltip title={t('Eliminate')} placement='top'>
                  <IconButton
                    ...
                    onClick={e => {
                      ...
                      blurActiveElement()
                      dispatch(registrationActions.setId(Number(params.row?.registration_id)))
                      dispatch(registrationActions.setShowEliminateDialog(true))
                    }}
                  >
  ...
  const fetchStudents = useCallback(async () => {
    if (!open || !active || !courseId) return
  ...
  }, [open, active, courseId, paginationModel.page, paginationModel.pageSize, sortModel])

  useEffect(() => {
    fetchStudents()
  }, [fetchStudents, filterButtonClickCount])
  ...
  const handleCreate = () => {
    ...
    dispatch(registrationActions.openModal({ mode: 'create', courseId: null }))
  }
      ...
      <CardContent>
        ...
        <Box sx={{ height: '60vh', width: '100%' }}>
          <DataGrid
            ...
            getRowId={row => row.id ?? row.student_id ?? row.registration_id ?? `${row.dni}-${row.name}`}
          />
        </Box>
      </CardContent>
