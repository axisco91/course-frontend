...
import LevelStudiesGeneralTab from './LevelStudiesGeneralTab'
...
const LevelStudiesModel: React.FC<LiquidationModalProps> = ({ open, onClose, mode, levelStudyId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Level Study') : t('Level Study')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <LevelStudiesGeneralTab
            open={open}
            mode={effectiveMode}
            levelStudyId={levelStudyId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default LevelStudiesModel
