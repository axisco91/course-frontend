...
// ** API
import { getLevelStudies } from 'src/api/api'
...
const LevelStudiesTable = () => {
  ...
  const appliedFilters = useSelector((state: RootState) => (state as any).levelStudy?.appliedFilters ?? {})
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        blurActiveElement()
                        dispatch(levelStudyActions.setId(id))
                        dispatch(levelStudyActions.openModal({ mode: 'edit' }))
                      }}
                    >
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        ...
                        blurActiveElement()
                        dispatch(levelStudyActions.setId(id))
                        dispatch(levelStudyActions.setShowEliminateDialog(true))
                      }}
                    >
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      ...
      const res = await getLevelStudies({
      ...
      setRows(res.data?.data?.level_studies ?? res.data?.data?.data ?? [])
    } catch (error) {
...
export default LevelStudiesTable
