...
const LiquidationsFilters = () => {
  const { t } = useTranslation()
  const dispatch = useDispatch()

  const userPermissions = useSelector((state: RootState) => state.auth.permissions) as string[]

  const canCreate = Array.isArray(userPermissions) && userPermissions.includes('create.liquidations')

  const handleCreate = () => {
    dispatch(liquidationActions.setId(null))

    // ✅ abre modal en create
    dispatch(liquidationActions.openModal({ mode: 'create', liquidationId: null }))
  }
...
export default LiquidationsFilters
