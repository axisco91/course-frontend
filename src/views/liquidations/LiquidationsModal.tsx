...
import LiquidationsGeneralTab from './LiquidationsGeneralTab'
...
const LiquidationsModel: React.FC<LiquidationModalProps> = ({ open, onClose, mode, liquidationId = null }) => {
  ...
  const hasId = Boolean(liquidationId)
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Liquidation') : t('Liquidation')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <LiquidationsGeneralTab
            open={open}
            mode={effectiveMode}
            liquidationId={liquidationId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default LiquidationsModel
