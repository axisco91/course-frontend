...
const ModalitiesFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).modality.filters)

  const handleCreate = () => {
    dispatch(modalityActions.setId(null))

    // ✅ abre modal en create
    dispatch(modalityActions.openModal({ mode: 'create', modalityId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(modalityActions.setFilter({ key, value }))
...
export default ModalitiesFilters
