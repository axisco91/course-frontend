...
// ✅ APIs
import { createModality, editModality, getModality } from 'src/api/api'
...
interface ModalitiesGeneralTabProps {
  open: boolean
  mode: Mode
...
const ModalitiesGeneralTab: React.FC<ModalitiesGeneralTabProps> = ({ open, mode, modalityId, onLoaded, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await getModality(modalityId)
        if (cancelled) return

        const l = res.data?.data?.modality ?? null
        if (!l) throw new Error('Modality payload not found')
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      if (mode === 'create') {
        const response = await createModality(formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())

          const newId = response.data?.data?.modality?.id ?? response.data?.data?.id
          if (newId) {
            if (modalityActions?.setId) dispatch(modalityActions.setId(Number(newId)))
            if (modalityActions?.openModal) {
              dispatch(modalityActions.openModal({ mode: 'edit', modalityId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && modalityId) {
        const response = await editModality(modalityId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default ModalitiesGeneralTab
