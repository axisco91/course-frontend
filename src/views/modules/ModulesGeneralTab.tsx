...
import { createModule, editModule, getModule } from 'src/api/api'
...
interface ModulesGeneralTabProps {
  open: boolean
  mode: Mode
  moduleId: number | null
  onLoaded?: (moduleData: any) => void
  onClose?: () => void
...
const schema = yup.object().shape({
  formative_module: yup.string().trim().required('Módulo Formativo es requerido'),
...
const isModuleLike = (value: any) => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false

  return (
    value.id !== undefined ||
    value.name !== undefined ||
    value.formative_module !== undefined ||
    value.code !== undefined ||
    value.total_hours !== undefined
  )
}

const findModuleInUnknown = (raw: any): any => {
  if (raw == null) return null

  if (Array.isArray(raw)) {
    for (const item of raw) {
      const found = findModuleInUnknown(item)
  ...
  if (isModuleLike(raw)) return raw

  const priorityKeys = ['module', 'modules', 'item', 'result', 'payload', 'data']
  for (const key of priorityKeys) {
    if (key in raw) {
      const found = findModuleInUnknown(raw[key])
      if (found) return found
    }
  }

  for (const value of Object.values(raw)) {
    const found = findModuleInUnknown(value)
    if (found) return found
  }
...
const extractModuleFromResponse = (data: any) => {
  ...
  for (const candidate of directCandidates) {
    if (Array.isArray(candidate)) {
      if (candidate.length && isModuleLike(candidate[0])) return candidate[0]
    } else if (isModuleLike(candidate)) {
      return candidate
    }
  }

  return findModuleInUnknown(data)
}
...
const mapModuleToFormValues = (moduleData: any): FormValues => {
  const tutoringHours = toNumberOrZero(moduleData?.tutoring_hours)
  const examHours = toNumberOrZero(moduleData?.exam_hours)
  const teletrainingHours = toNumberOrZero(moduleData?.teletraining_hours)
  ...
  return {
    formative_module: String(moduleData?.formative_module ?? moduleData?.code ?? ''),
    name: String(moduleData?.name ?? ''),
    tutoring_hours: toNumberOrNull(moduleData?.tutoring_hours) ?? 0,
    exam_hours: toNumberOrNull(moduleData?.exam_hours) ?? 0,
    teletraining_hours: toNumberOrNull(moduleData?.teletraining_hours) ?? 0,
    face_to_face_hours: toNumberOrNull(moduleData?.face_to_face_hours) ?? computedFaceToFace,
    total_hours: toNumberOrNull(moduleData?.total_hours ?? moduleData?.hours) ?? computedTotal,
    active: toBooleanActive(moduleData?.active ?? moduleData?.is_active)
  }
}

const ModulesGeneralTab: React.FC<ModulesGeneralTabProps> = ({
  open,
  mode,
  moduleId,
  onLoaded,
  ...
  useEffect(() => {
    ...
    const localModule = toArray(modules).find((item: any) => Number(item?.id) === Number(moduleId))
    if (localModule) {
      onLoaded?.(localModule)
      dispatch(moduleActions.setName(localModule?.name ?? ''))
      reset(mapModuleToFormValues(localModule))
    }
    ...
    const loadModule = async () => {
      ...
      try {
        ...
        const moduleData = extractModuleFromResponse(res.data)
        if (!moduleData) throw new Error('Module payload not found')

        onLoaded?.(moduleData)
        dispatch(moduleActions.setName(moduleData?.name ?? ''))
        reset(mapModuleToFormValues(moduleData))
      } catch (error) {
  ...
  }, [open, mode, moduleId, modules, defaultValues, dispatch, onLoaded, reset])
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      formData.append('formative_module', data.formative_module?.trim() ?? '')
      formData.append('code', data.formative_module?.trim() ?? '')
      ...
      if (mode === 'create') {
        ...
        if (response.data?.success) {
          toast.success(response.data?.message ?? 'Módulo guardado')
          dispatch(generalActions.addFilterButtonClickCount())

          const newModule = extractModuleFromResponse(response.data)
          if (newModule) {
            dispatch(moduleActions.setId(Number(newModule.id ?? 0) || null))
            dispatch(moduleActions.setName(String(newModule.name ?? data.name ?? '')))
          }

          onClose?.()
        } else {
          toast.error(response.data?.message ?? 'Error al guardar el módulo')
        }
      } else if (mode === 'edit' && moduleId) {
        const response = await editModule(moduleId, formData)

        if (response.data?.success) {
          toast.success(response.data?.message ?? 'Módulo guardado')
          dispatch(generalActions.addFilterButtonClickCount())
          dispatch(moduleActions.setName(data.name?.trim() ?? ''))
          onClose?.()
        } else {
          toast.error(response.data?.message ?? 'Error al guardar el módulo')
        }
      ...
      <form onSubmit={handleSubmit(onFormSubmit)}>
        <Grid container spacing={5}>
          <Grid item xs={12} md={4}>
            <Controller
              name='formative_module'
              control={control}
              render={({ field }) => (
                <CustomTextField
                  fullWidth
                  label='Módulo Formativo'
                  placeholder='Módulo Formativo'
                  error={Boolean(errors.formative_module)}
                  helperText={errors.formative_module?.message}
                  disabled={readOnly}
                  {...field}
                />
              )}
...
export default ModulesGeneralTab
