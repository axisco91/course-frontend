...
import NacionalFestivalsGeneralTab from './NacionalFestivalsGeneralTab'
...
interface NacionalFestivalModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const NacionalFestivalsModel: React.FC<NacionalFestivalModalProps> = ({
  open,
  onClose,
  mode,
  nacionalFestivalId = null
}) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Nacional Festival') : t('Nacional Festival')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <NacionalFestivalsGeneralTab
            open={open}
            mode={effectiveMode}
            nacionalFestivalId={nacionalFestivalId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default NacionalFestivalsModel
