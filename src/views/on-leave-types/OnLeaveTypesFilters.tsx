...
const OnLeaveTypesFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).onLeave.filters)

  const handleCreate = () => {
    dispatch(onLeaveActions.setId(null))

    // ✅ abre modal en create
    dispatch(onLeaveActions.openModal({ mode: 'create', onLeaveId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(onLeaveActions.setFilter({ key, value }))
...
export default OnLeaveTypesFilters
