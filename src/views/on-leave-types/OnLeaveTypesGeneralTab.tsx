...
// ✅ APIs
import { createOnLeaveType, editActionType, editOnLeaveType, getOnLeaveType } from 'src/api/api'
...
interface ActionTypeGeneralTabProps {
  open: boolean
  mode: Mode
  onLeaveId: number | null
  onLoaded?: (liquidation: any) => void
  onClose?: () => void
}

const OnLeaveTypesGeneralTab: React.FC<ActionTypeGeneralTabProps> = ({ open, mode, onLeaveId, onLoaded, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await getOnLeaveType(onLeaveId)

        if (cancelled) return

        const l = res.data?.data?.on_leave_type ?? null
        if (!l) throw new Error('On Leave Type payload not found')
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      if (mode === 'create') {
        const response = await createOnLeaveType(formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())

          const newId = response.data?.data?.on_leave_type?.id ?? response.data?.data?.id
          if (newId) {
            if (onLeaveActions?.setId) dispatch(onLeaveActions.setId(Number(newId)))
            if (onLeaveActions?.openModal) {
              dispatch(onLeaveActions.openModal({ mode: 'edit', onLeaveId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && onLeaveId) {
        const response = await editOnLeaveType(onLeaveId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default OnLeaveTypesGeneralTab
