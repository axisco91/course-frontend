...
// ✅ APIs
import { createPayment, editPayment, getPayment } from 'src/api/api'
...
interface PaymentsGeneralTabProps {
  open: boolean
  mode: Mode
...
const PaymentsGeneralTab: React.FC<PaymentsGeneralTabProps> = ({ open, mode, paymentId, onLoaded, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const l = res.data?.data?.payment ?? null
        if (!l) throw new Error('Payment payload not found')
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (response.data?.success) {
          ...
          const newId = response.data?.data?.payment?.id ?? response.data?.data?.id
          if (newId) {
            if (paymentActions?.setId) dispatch(paymentActions.setId(Number(newId)))
            if (paymentActions?.openModal) {
              dispatch(paymentActions.openModal({ mode: 'edit', paymentId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && paymentId) {
        const response = await editPayment(paymentId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default PaymentsGeneralTab
