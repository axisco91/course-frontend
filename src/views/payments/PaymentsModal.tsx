...
import PaymentsGeneralTab from './PaymentsGeneralTab'
...
const PaymentsModel: React.FC<PaymentModalProps> = ({ open, onClose, mode, paymentId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Payment') : t('Payment')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <PaymentsGeneralTab open={open} mode={effectiveMode} paymentId={paymentId ?? null} onClose={onClose} />
        </TabPanel>
      </DialogContent>
    </Dialog>
  )
