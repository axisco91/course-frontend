...
import PopulationFestivalsGeneralTab from './PopulationFestivalsGeneralTab'
...
interface PopulationFestivalsModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const PopulationFestivalsModal: React.FC<PopulationFestivalsModalProps> = ({
  open,
  onClose,
  mode,
  populationFestivalId = null
}) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Population Festival') : t('Population Festival')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <PopulationFestivalsGeneralTab
            open={open}
            mode={effectiveMode}
            populationFestivalId={populationFestivalId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default PopulationFestivalsModal
