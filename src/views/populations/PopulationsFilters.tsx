...
const PopulationsFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).population.filters)

  const handleCreate = () => {
    dispatch(populationActions.setId(null))

    // ✅ abre modal en create
    dispatch(populationActions.openModal({ mode: 'create', populationId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(populationActions.setFilter({ key, value }))
...
export default PopulationsFilters
