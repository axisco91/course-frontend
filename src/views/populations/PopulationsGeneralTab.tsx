...
// ✅ APIs
import { createPopulation, editPopulation, getPopulation } from 'src/api/api'
...
interface PopulationsGeneralTabProps {
  open: boolean
  mode: Mode
...
const PopulationsGeneralTab: React.FC<PopulationsGeneralTabProps> = ({
  open,
  mode,
  populationId,
  onLoaded,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await getPopulation(populationId)
        if (cancelled) return

        const l = res.data?.data?.population ?? null
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      if (mode === 'create') {
        const response = await createPopulation(formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())

          const newId = response.data?.data?.population?.id ?? response.data?.data?.id
          if (newId) {
            if (populationActions?.setId) dispatch(populationActions.setId(Number(newId)))
            if (populationActions?.openModal) {
              dispatch(populationActions.openModal({ mode: 'edit', populationId: Number(newId) }))
            }
          }
        }
      } else if (mode === 'edit' && populationId) {
        const response = await editPopulation(populationId, formData)
        if (response.data?.success) {
          toast.success(response.data?.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
...
export default PopulationsGeneralTab
