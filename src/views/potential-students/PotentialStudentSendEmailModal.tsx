...
import { sendPotentialstudent } from 'src/api/api'
...
const PotentialStudentSendEmailModal = () => {
  const { t } = useTranslation()
  const dispatch = useDispatch()
  const { handleError } = useErrorHandler()
  const { logout } = useContext(AuthContext)
  const open = useSelector((state: RootState) => (state.potentialStudent as any).showSendEmailModal)
  ...
  const close = () => {
    dispatch(potentialStudentActions.changeSendEmailModalStatus())
    reset({ email: '' })
  }
  ...
  const onSubmit = async (data: { email: string }) => {
    ...
    try {
      ...
      const response = await sendPotentialstudent(formData)
...
export default PotentialStudentSendEmailModal
