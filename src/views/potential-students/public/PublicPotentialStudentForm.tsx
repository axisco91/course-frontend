...
import {
  createPotentialStudentPublic,
  getProfessionalCategoriesPublic,
  getProvinces,
  potentialStudentCheckDniPublic
} from 'src/api/api'
...
type FormValues = {
  ...
  professional_category_id: number | null
  direction: string
  post_code: string
...
const PublicPotentialStudentForm = () => {
  const { t } = useTranslation()
  const dispatch = useDispatch()
  const router = useRouter()
  const [saving, setSaving] = useState(false)
  const [disabled, setDisabled] = useState(false)
  const [professionalCategories, setProfessionalCategories] = useState<Option[]>([])
  ...
  useEffect(() => {
    ...
    const loadCatalogs = async () => {
      try {
        const [categoriesRes, provincesRes] = await Promise.all([getProfessionalCategoriesPublic(), getProvinces()])
        if (cancelled) return
        setProfessionalCategories(categoriesRes?.data?.data?.professional_categories ?? [])
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      formData.append(
        ...
        data.professional_category_id != null ? String(data.professional_category_id) : ''
      )
      ...
      dispatch(potentialStudentActions.setStudentSuccess(true))
      await router.push('/potential-student/finalized')
    } catch (error: any) {
      toast.error(error?.response?.data?.message || t('unexpectedError'))
    } finally {
      setSaving(false)
      ...
      <CardContent>
        <Box component='form' onSubmit={handleSubmit(onSubmit)}>
          <Grid container spacing={5}>
            ...
            <Grid item xs={12} md={4}>
              <Controller name='company' control={control} render={({ field }) => <CustomTextField fullWidth label={t('Company')} {...field} />} />
            </Grid>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete
                    ...
                    value={findById(professionalCategories, field.value)}
                    onChange={(_, value) => field.onChange(value?.id ?? null)}
                    getOptionLabel={option => option?.name ?? ''}
                    isOptionEqualToValue={(o, v) => Number(o.id) === Number(v.id)}
                    renderInput={params => <CustomTextField {...params} label={t('Professional Category')} />}
                  />
                )}
              />
            </Grid>
...
export default PublicPotentialStudentForm
