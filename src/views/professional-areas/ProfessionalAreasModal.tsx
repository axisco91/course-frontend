...
import ProfessionalAreasGeneralTab from './ProfessionalAreasGeneralTab'
...
interface ProfessionalAreaModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const ProfessionalAreasModel: React.FC<ProfessionalAreaModalProps> = ({
  open,
  onClose,
  mode,
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Professional Area') : t('Professional Area')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <ProfessionalAreasGeneralTab
            open={open}
            mode={effectiveMode}
            professionalAreaId={professionalAreaId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default ProfessionalAreasModel
