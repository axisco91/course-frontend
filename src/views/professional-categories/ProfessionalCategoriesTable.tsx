...
import { getProfessionalCategories } from 'src/api/api'

type SortType = 'asc' | 'desc' | undefined | null

const ProfessionalCategoriesTable = () => {
  ...
  const appliedFilters = useSelector((state: RootState) => (state as any).professionalCategory?.appliedFilters ?? {})
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        blurActiveElement()
                        dispatch(professionalCategoryActions.setId(id))
                        dispatch(professionalCategoryActions.openModal({ mode: 'edit' }))
                      }}
                    >
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        ...
                        blurActiveElement()
                        dispatch(professionalCategoryActions.setId(id))
                        dispatch(professionalCategoryActions.setShowEliminateDialog(true))
                      }}
                    >
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      ...
      const res = await getProfessionalCategories({
      ...
      setRows(res.data?.data?.professional_categories ?? res.data?.data?.data ?? [])
    } catch (error) {
...
export default ProfessionalCategoriesTable
