...
import { getProfitStudents } from 'src/api/api'
...
const ProfitsStudentsTable = ({ open, profitId }: Props) => {
  ...
  const ready = open && Number(profitId) > 0

  // ✅ resetea cuando no está listo (evita estados “viejos”)
  useEffect(() => {
    if (!ready) {
      setRows([])
      setTotal(0)
  ...
  const fetchStudents = useCallback(async () => {
    // LOG mínimo para saber por qué no entra
    // eslint-disable-next-line no-console
    console.log('[ProfitsStudentsTable] fetchStudents called', { open, profitId, ready })
    ...
    try {
      ...
      const res = await getProfitStudents(Number(profitId), {
  ...
  }, [open, profitId, ready, paginationModel.page, paginationModel.pageSize, sortModel])

  // ✅ este effect SOLO depende de ready + modelos (lo típico en server-side grid)
  useEffect(() => {
    if (!ready) return
    fetchStudents()
  }, [ready, fetchStudents])
