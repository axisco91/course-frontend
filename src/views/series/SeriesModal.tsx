...
import SeriesGeneralTab from './SeriesGeneralTab'
...
const SeriesModel: React.FC<SerieModalProps> = ({ open, onClose, mode, serieId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Series') : t('Series')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <SeriesGeneralTab open={open} mode={effectiveMode} serieId={serieId ?? null} onClose={onClose} />
        </TabPanel>
      </DialogContent>
    </Dialog>
  )
