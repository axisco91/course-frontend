...
import { getTrainingContractSeries } from 'src/api/api'
...
const SeriesTable = () => {
  ...
  const appliedFilters = useSelector((state: RootState) => (state as any).trainingContractSerie?.appliedFilters ?? {})
    ...
    () => [
      {
        ...
        renderCell: (params: GridRenderCellParams) => (
          <Typography noWrap variant='body2' sx={{ color: 'text.primary', fontWeight: 600 }}>
            {params.row?.series ?? params.row?.name ?? ''}
          </Typography>
        )
      },
      {
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        blurActiveElement()
                        dispatch(trainingContractSerieActions.setId(id))
                        dispatch(trainingContractSerieActions.openModal({ mode: 'edit' }))
                      }}
                    >
                  ...
                  <span>
                    <IconButton
                      ...
                      onClick={e => {
                        ...
                        blurActiveElement()
                        dispatch(trainingContractSerieActions.setId(id))
                        dispatch(trainingContractSerieActions.setShowEliminateDialog(true))
                      }}
                    >
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      ...
      const res = await getTrainingContractSeries({
      ...
      setRows(res.data?.data?.training_contract_series ?? res.data?.data?.data ?? [])
    } catch (error) {
