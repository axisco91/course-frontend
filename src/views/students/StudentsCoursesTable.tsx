...
import { getStudentCourses } from 'src/api/api'
import Icon from 'src/@core/components/icon'

// ✅ redux
import { useDispatch, useSelector } from 'react-redux'
import { RootState } from 'src/reducers/types/types'
import { courseActions } from 'src/reducers/courses/CourseReducer' // <-- ajusta path/nombre real
...
const StudentsCoursesTable = ({ open, studentId, active = true }: Props) => {
          ...
          return (
            <Tooltip title={t('View')} placement='top'>
              <IconButton
                ...
                onClick={e => {
                  e.stopPropagation()
                  const id = Number(courseIdFromRow)
                  if (!id) return

                  dispatch(courseActions.setId(id))
  ...
  // ✅ para ignorar respuestas viejas si cambias rápido de tab/página
  const reqIdRef = useRef(0)

  const fetchCourses = useCallback(async () => {
    if (!open || !active || !studentId) return
  ...
  }, [open, active, studentId, paginationModel.page, paginationModel.pageSize, sortModel])
      ...
      <CardContent>
        <Box sx={{ height: '60vh', width: '100%' }}>
          <DataGrid
            ...
            getRowId={row => row.registration_id ?? row.id ?? `${row.course_id}-${row.student_id}-${row.beginning}`}
          />
        </Box>
      </CardContent>
