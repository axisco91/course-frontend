...
import { createStudent, editStudent, getStudent, studentCheckDni } from 'src/api/api'
...
const normalizeStr = (value: any) => String(value ?? '').trim().toLowerCase()
...
const parseDateValue = (value: any): Date | null => {
  ...
  const fallback = new Date(Number(yyyy), Number(mm) - 1, Number(dd))
...
const pickByIdOrName = (list: List[], idCandidate: any, nameCandidate: any) => {
  ...
  const fallbackName = String(nameCandidate ?? '').trim()
  if (!fallbackName) return null

  return { id: Number.isFinite(id) && id > 0 ? id : 0, name: fallbackName, label: fallbackName } as List & {
    label: string
  }
}
...
const CustomInput = forwardRef<HTMLInputElement, CustomInputProps>(
  ({ value, label, onClick, onChange, onBlur, disabled }, ref) => (
    <CustomTextField
      fullWidth
      inputRef={ref}
      label={label}
...
interface StudentsGeneralTabProps {
  open: boolean
  mode: Mode
...
const StudentsGeneralTab: React.FC<StudentsGeneralTabProps> = ({ open, mode, studentId, onLoadedStudent, onClose }) => {
  ...
  const showCompanyEye = Boolean(studentId) && canReadCompanies
  ...
  const [loadedStudentData, setLoadedStudentData] = useState<any | null>(null)
  ...
  // ✅ CARGA DEL ALUMNO (sin dependencias que provoquen loops)
  useEffect(() => {
    if (!open) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const s = res.data?.data?.student ?? res.data?.data ?? res.data?.student ?? null
        if (!s) throw new Error('Student payload not found')

        setLoadedStudentData(s)
        onLoadedStudent?.(s)
      } catch (error) {
  ...
  }, [open, studentId, mode === 'create'])

  useEffect(() => {
    if (!open || mode === 'create' || !loadedStudentData || isDirty) return

    const s = loadedStudentData
    ...
    const provinceNameFromApi =
      typeof s.province === 'string' ? s.province : s.province?.name ?? s.province_name ?? s.provinceName ?? s.province?.label ?? null
    ...
    const companyNameFromApi =
      typeof s.company === 'string' ? s.company : s.company?.name ?? s.company_name ?? s.companyName ?? s.company?.label ?? null
    ...
    const levelStudyNameFromApi =
      typeof s.level_study === 'string'
        ? s.level_study
        : s.level_study?.name ?? s.level_study_name ?? s.levelStudyName ?? s.study_level ?? s.level_study?.label ?? null
  ...
  const onFormSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      if (dni.length > 0) {
        const check = await studentCheckDni({ dni: dni, id: studentId ? String(studentId) : '' })

        if (check.data?.exists) {
          toast.error(t('DNI ya en uso'))

          // si usas redux flag:
          // dispatch(studentActions.setDniExist(true))
          return // 🔴 corta aquí, no guarda
        }

        // dispatch(studentActions.setDniExist(false))
      }
    } catch (error) {
    ...
    try {
      ...
      formData.append('company_id', Number(data.company?.id) > 0 ? String(data.company.id) : '')
      ...
      if (mode === 'create') {
        ...
        if (response.data?.success) {
          ...
          const newIdRaw = response.data?.data?.student?.id ?? response.data?.data?.id
          const newId = newIdRaw != null ? Number(newIdRaw) : null

          if (newId && !Number.isNaN(newId)) {
            dispatch(studentActions.setId(newId))
            dispatch(studentActions.openStudentModal({ mode: 'edit', studentId: newId }))
          }
        }
      } else if (mode === 'edit' && studentId) {
        const response = await editStudent(studentId, formData)
        if (response.data?.success) {
          toast.success(response.data.message ?? t('Saved'))
          dispatch(generalActions.addFilterButtonClickCount())
        }
  ...
  const openCompanyModal = (companyId?: number | null) => {
    const normalizedCompanyId = companyId != null ? Number(companyId) : null
    dispatch(companyActions.setId(normalizedCompanyId))

    // abre modal en modo view
    dispatch(companyActions.openModal({ mode: 'view', companyId: normalizedCompanyId }))
  }
            ...
            <Controller
              ...
              render={({ field }) => (
                <DatePicker
                  ...
                  onBlur={field.onBlur}
                  customInput={<CustomInput label={t('Date of Birth')} disabled={disabled} />}
                  calendarStartDay={1}
                    ...
                    <CustomTextField
                      ...
                      label={
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          {showCompanyEye && Number(field.value?.id) > 0 && (
                            <Box
                              role='button'
...
export default StudentsGeneralTab
