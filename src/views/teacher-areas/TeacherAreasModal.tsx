...
import TeacherAreasGeneralTab from './TeacherAreasGeneralTab'
...
const TeacherAreasModel: React.FC<TeacherAreaModalProps> = ({ open, onClose, mode, teacherAreaId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Teacher Area') : t('Teacher Area')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <TeacherAreasGeneralTab
            open={open}
            mode={effectiveMode}
            teacherAreaId={teacherAreaId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default TeacherAreasModel
