...
import TracingsGeneralTab from './TracingsGeneralTab'

// ✅ Usa tus componentes reales de Tracings
...
const TracingsModal: React.FC<TracingsModalProps> = ({ open, onClose, mode, tracingId = null }) => {
  ...
  const handleLoaded = useCallback((tr: any) => {
    // ✅ Tracing title (ajusta el campo a tu modelo real)
    setTracingName(`${tr?.name ?? tr?.title ?? ''}`.trim())
  }, [])
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Tracing') : tracingName || t('Tracing')}
          </Typography>
        </Box>
