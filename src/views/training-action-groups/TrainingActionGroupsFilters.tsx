...
const TrainingActionGroupsFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).trainingActionGroup.filters)

  const handleCreate = () => {
    dispatch(trainingActionGroupActions.setId(null))

    // ✅ abre modal en create
    dispatch(trainingActionGroupActions.openModal({ mode: 'create', trainingActionGroupId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(trainingActionGroupActions.setFilter({ key, value }))
...
export default TrainingActionGroupsFilters
