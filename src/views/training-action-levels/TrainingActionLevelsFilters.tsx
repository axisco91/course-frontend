...
const TrainingActionLevelsFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).trainingActionLevel.filters)

  const handleCreate = () => {
    dispatch(trainingActionLevelActions.setId(null))

    // ✅ abre modal en create
    dispatch(trainingActionLevelActions.openModal({ mode: 'create', trainingActionLevelId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(trainingActionLevelActions.setFilter({ key, value }))
...
export default TrainingActionLevelsFilters
