...
import TrainingContractStatusesGeneralTab from './TrainingContractStatusesGeneralTab'
...
interface TrainingContractStatusesModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const TrainingContractStatusesModal: React.FC<TrainingContractStatusesModalProps> = ({
  open,
  onClose,
  mode,
  trainingContractStatusId = null
}) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Training Contract Status') : t('Training Contract Status')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <TrainingContractStatusesGeneralTab
            open={open}
            mode={effectiveMode}
            trainingContractStatusId={trainingContractStatusId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default TrainingContractStatusesModal
