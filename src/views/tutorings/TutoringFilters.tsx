...
const TutoringsFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).tutoring.filters)

  const handleCreate = () => {
    dispatch(tutoringActions.setId(null))

    // ✅ abre modal en create
    dispatch(tutoringActions.openModal({ mode: 'create', tutoringId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(tutoringActions.setFilter({ key, value }))
...
export default TutoringsFilters
