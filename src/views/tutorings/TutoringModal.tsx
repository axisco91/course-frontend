...
import TutoringsGeneralTab from './TutoringGeneralTab'
...
const TutoringsModel: React.FC<TutoringModalProps> = ({ open, onClose, mode, tutoringId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Tutoring') : t('Tutoring')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <TutoringsGeneralTab open={open} mode={effectiveMode} tutoringId={tutoringId ?? null} onClose={onClose} />
        </TabPanel>
      </DialogContent>
    </Dialog>
  )
