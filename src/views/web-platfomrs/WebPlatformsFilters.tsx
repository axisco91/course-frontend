...
const WebPlatformsFilters = () => {
  ...
  const filters = useSelector((state: RootState) => (state as any).webPlatform.filters)

  const handleCreate = () => {
    dispatch(webPlatformActions.setId(null))

    // ✅ abre modal en create
    dispatch(webPlatformActions.openModal({ mode: 'create', webPlatformId: null }))
  }

  const setFilter = (key: string, value: any) => dispatch(webPlatformActions.setFilter({ key, value }))
...
export default WebPlatformsFilters
