...
import WebPlatformsGeneralTab from './WebPlatformsGeneralTab'
...
interface WebPlatformModalProps {
  open: boolean
  onClose: () => void
  mode: Mode
...
const WebPlatformsModel: React.FC<WebPlatformModalProps> = ({ open, onClose, mode, webPlatformId = null }) => {
      ...
      <DialogContent sx={{ pt: 6 }}>
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Typography variant='h5' sx={{ fontWeight: 700 }}>
            {isCreate ? t('New Web Platform') : t('Web Platform')}
          </Typography>
        </Box>
        ...
        <TabPanel value={tab} index={0}>
          <WebPlatformsGeneralTab
            open={open}
            mode={effectiveMode}
            webPlatformId={webPlatformId ?? null}
            onClose={onClose}
          />
        </TabPanel>
      </DialogContent>
...
export default WebPlatformsModel
